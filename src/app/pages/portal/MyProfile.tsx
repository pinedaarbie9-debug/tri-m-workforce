// src/app/pages/portal/MyProfile.tsx
...
export function MyProfilePage() {
  ...
  // 🔒 Save Changes — WALANG password (reverted sa normal)
    ...
    <div className="p-6 space-y-6 max-w-2xl">
      ...
      >
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <UserCircle className="w-7 h-7" /> My Profile
        </h1>
        <p className="text-white/70 text-sm mt-1">
          Tingnan at i-update ang ilang detalye ng iyong profile
        </p>
      </motion.div>
          ...
          <div className="bg-card rounded-2xl p-5 border border-border shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <p className="font-medium font-mono">{profile.employee_code}</p>
              </div>
              <div>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              {saved && (
                <div className="flex items-center gap-2 p-2.5 bg-emerald-50 border border-emerald-200 rounded-lg text-sm text-emerald-700">
                  <CheckCircle2 className="w-4 h-4" /> Na-save na ang mga
                  pagbabago.
                </div>
              )}
              ...
              <button
                type="submit"
                disabled={saving}
                className="flex items-center gap-2 px-4 py-2.5 text-sm bg-primary text-white rounded-lg hover:bg-primary/90 disabled:opacity-60 transition-colors"
              >
