    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay, duration: 0.35 }}
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-2 text-2xl sm:text-3xl font-bold text-foreground truncate">{value}</p>
          {trend !== undefined && (
        ...
        <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: `${color}15`, color }}>
      ...
      >
        <div className="relative z-10 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            <p className="text-white/70 text-xs sm:text-sm">{format(now, "EEEE, MMMM d, yyyy")}</p>
            ...
            <p className="text-white/70 text-xs sm:text-sm mt-1 leading-relaxed">
              Automated Report Generation · Real-Time HR Monitoring · Support Evaluation · Web-Based
            </p>
          </div>

          <div className="grid grid-cols-2 gap-2 sm:gap-3 shrink-0">
            <div className="bg-white/10 backdrop-blur-sm rounded-xl px-3 py-2 sm:px-4 sm:py-3 border border-white/20 text-center">
              <p className="text-white/60 text-[10px] sm:text-xs">Today's Attendance</p>
              <p className="text-white font-bold text-lg sm:text-2xl">
                {loading ? "…" : `${stats?.attendanceRate ?? 0}%`}
              </p>
            </div>
            <div className="bg-white/10 backdrop-blur-sm rounded-xl px-3 py-2 sm:px-4 sm:py-3 border border-white/20 text-center">
              <p className="text-white/60 text-[10px] sm:text-xs">Active Employees</p>
              <p className="text-white font-bold text-lg sm:text-2xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }}
              ...
              {(!stats.monthlyTrend || stats.monthlyTrend.length === 0) ? (
                <p className="text-sm text-muted-foreground text-center py-10">No attendance data yet for the trend.</p>
              ) : (
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
              ...
              {shiftDistribution.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-10">No shifts scheduled today.</p>
              ) : (
                <>
              ...
              <table className="w-full min-w-[600px]">
                <thead>
                  <tr className="border-b border-border">
                    <th className="text-left px-4 sm:px-5 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Employee</th>
                    <th className="text-left px-4 sm:px-5 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Department</th>
                    <th className="text-left px-4 sm:px-5 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Action</th>
                    <th className="text-left px-4 sm:px-5 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Time</th>
                    <th className="text-left px-4 sm:px-5 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Status</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border">
                  {recentActivity.length === 0 && (
                    <tr><td colSpan={5} className="px-5 py-10 text-center text-sm text-muted-foreground">No recent activity found.</td></tr>
                  )}
                  {recentActivity.map((item) => {
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-muted/30 transition-colors">
                        ...
                        <td className="px-4 sm:px-5 py-3.5 text-sm text-muted-foreground">{item.employee?.department?.name ?? "—"}</td>
                        <td className="px-4 sm:px-5 py-3.5 text-sm text-foreground capitalize">{item.method} {item.type.replace("_", " ")}</td>
                        <td className="px-4 sm:px-5 py-3.5 text-sm text-muted-foreground font-mono">{format(new Date(item.timestamp), "h:mm a")}</td>
                        <td className="px-4 sm:px-5 py-3.5">
