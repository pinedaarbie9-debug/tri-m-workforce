...
export function SearchResultsPage() {
  ...
  const matchedEmployees = useMemo(() => {
    ...
    return employees.filter((e: any) => {
      const haystack = [
        e.full_name, e.first_name, e.last_name, e.email, e.job_title,
        e.position, e.department?.name, e.department_name,
      ]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
      <div>
        <h1 className="text-xl sm:text-2xl font-medium text-foreground flex items-center gap-2 break-words">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          {employeesLoading
            ? "Searching..."
            : `${matchedEmployees.length + matchedReports.length} results found`}
        </p>
      </div>
