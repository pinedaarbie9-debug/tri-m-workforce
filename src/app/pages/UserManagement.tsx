...
export function UserManagementPage() {
  ...
  async function handleAddUser(e: React.FormEvent) {
    ...
    if (!form.full_name.trim() || !form.email.trim() || form.password.length < 6) {
      setFormError("Full name, email, and password (min 6 characters) are required.");
      return;
    }
    if (form.role === "employee" && !form.employee_id) {
      setFormError("An employee record must be linked for the 'Employee' role.");
      return;
    }
    setSaving(true);
    try {
  ...
  async function handleEditUser(e: React.FormEvent) {
    ...
    if (editForm.password && editForm.password.length < 6) {
      setEditError("If changing the password, it must be at least 6 characters.");
      return;
    }
    if (editForm.role === "employee" && !editForm.employee_id) {
      setEditError("An employee record must be linked for the 'Employee' role.");
      return;
    }
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            <h1 className="text-xl sm:text-2xl font-bold flex items-center gap-2">
              <ShieldCheck className="w-6 h-6 sm:w-7 sm:h-7" /> User Management
            </h1>
            <p className="text-white/70 text-xs sm:text-sm mt-1">Manage system users, roles and permissions</p>
          </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex gap-1 bg-muted rounded-xl p-1 w-fit">
          {(["users", "roles"] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)} className={`px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors capitalize ${tab === t ? "bg-card shadow text-foreground" : "text-muted-foreground hover:text-foreground"}`}>
              ...
              <table className="w-full min-w-[700px]">
                <thead>
                  <tr className="border-b border-border bg-muted/30">
                    ...
                    <th className="text-left px-4 sm:px-5 py-3.5 text-xs font-semibold text-muted-foreground uppercase tracking-wide whitespace-nowrap">Role</th>
                    ...
                    <th className="text-left px-4 sm:px-5 py-3.5 text-xs font-semibold text-muted-foreground uppercase tracking-wide whitespace-nowrap">Last Login</th>
                    <th className="px-4 sm:px-5 py-3.5" />
                  </tr>
                </thead>
                <tbody className="divide-y divide-border">
                  {filtered.length === 0 && (
                    <tr><td colSpan={6} className="px-5 py-10 text-center text-sm text-muted-foreground">No users found.</td></tr>
                  )}
                  {filtered.map((user) => {
                      ...
                      <tr key={user.id} className="hover:bg-muted/20 transition-colors">
                        <td className="px-4 sm:px-5 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                              <p className="text-sm font-medium text-foreground whitespace-nowrap">{user.full_name}</p>
                              <p className="text-xs text-muted-foreground truncate">{user.email}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-4 sm:px-5 py-4 text-sm text-muted-foreground whitespace-nowrap">{user.department ?? "—"}</td>
                        <td className="px-4 sm:px-5 py-4">
                          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium border whitespace-nowrap ${roleCfg.className}`}>
                            <RoleIcon className="w-3 h-3" /> {roleCfg.label}
                          </span>
                        </td>
                        <td className="px-4 sm:px-5 py-4">
                          <span className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize whitespace-nowrap ${statusColors[user.status]}`}>
                            {user.status}
                          </span>
                        </td>
                        <td className="px-4 sm:px-5 py-4 text-sm text-muted-foreground font-mono whitespace-nowrap">{user.last_login ?? "—"}</td>
                        <td className="px-4 sm:px-5 py-4">
                          <DropdownMenu>
            ...
            return (
              <motion.div key={role} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}
                ...
                <div className="flex items-center gap-3 mb-4">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center border shrink-0 ${cfg.className}`}>
                    <Icon className="w-5 h-5" />
                  </div>
                  <div className="min-w-0">
                    <p className="font-semibold text-foreground truncate">{cfg.label}</p>
        ...
        <DialogContent className="sm:max-w-md max-h-[85vh] overflow-y-auto">
          ...
          <form onSubmit={handleAddUser} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              {form.role === "employee" && (
                <p className="text-xs text-muted-foreground">Required for the user to access the Employee Portal.</p>
              )}
            </div>
      ...
      <Dialog open={showEditModal} onOpenChange={(open) => { if (!open) closeEditModal(); }}>
        <DialogContent className="sm:max-w-md max-h-[85vh] overflow-y-auto">
          ...
          <form onSubmit={handleEditUser} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-foreground">Email (used for login)</label>
            ...
            <div className="space-y-1.5">
              ...
              <select value={editForm.status} onChange={(e) => setEditForm({ ...editForm, status: e.target.value as UserRow["status"] })}
                className="w-full px-3.5 py-2.5 rounded-lg border border-border bg-input-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/20">
            ...
            <div className="space-y-1.5">
              ...
              <select value={editForm.employee_id} onChange={(e) => setEditForm({ ...editForm, employee_id: e.target.value })} disabled={loadingLinkedEmployee}
              ...
              {loadingLinkedEmployee && (
                <p className="text-xs text-muted-foreground">Fetching linked employee record...</p>
              )}
              {linkedDeletedEmployee && !loadingLinkedEmployee && (
                <p className="text-xs text-amber-600">
                  Note: The linked employee has been deleted. Select another employee or change the role if needed.
                </p>
              )}
            </div>
            <div className="flex flex-col sm:flex-row justify-end gap-2 pt-2">
