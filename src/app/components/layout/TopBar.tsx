...
export function TopBar({ onMenuClick }: TopBarProps) {
  ...
  return (
    <header className="h-16 bg-card border-b border-border flex items-center px-3 sm:px-6 gap-2 sm:gap-4 shrink-0 shadow-sm">
      {onMenuClick && (
        <button
          type="button"
          onClick={onMenuClick}
          aria-label="Open navigation menu"
          className="lg:hidden w-9 h-9 rounded-lg flex items-center justify-center hover:bg-muted transition-colors focus:outline-none focus:ring-2 focus:ring-primary/40 shrink-0"
        >
